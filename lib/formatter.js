...
function format(mixed) {
  var mixedType = utility.getTypeName(mixed),
      records = [],
      i, len;

  if (mixedType !== 'object') {
    throw new TypeError(
        'parameter should be a Object or Array: ' + mixedType);
  }
...
function objectToRecord_(object) {
  var objectType = utility.getTypeName(object),
      keys, fields, i, len;

  if (objectType !== 'object') {
    throw new TypeError(
        'parameter should be a Object: ' + objectType);
  }
  ...
  for (i = 0, len = keys.length; i < len; ++i) {
    ...
    if (!validator.isValidLabel(label)) {
      throw new SyntaxError(
          'unexpected character of label: "' + label + '"');
    }

    if (!validator.isValidValue(value)) {
      throw new SyntaxError(
          'unexpected character of value: "' + value + '"');
    }
