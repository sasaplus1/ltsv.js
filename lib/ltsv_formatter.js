...
function format_(ltsvList, check) {
  var i, len, result;
  ...
  for (i = 0, len = ltsvList.length; i < len; ++i) {
    result[i] = formatLine(ltsvList[i], check);
  }

  return result.join('\n');
}

function formatLine_(recordObj, check) {
  ...
  for (i = 0, len = keys.length; i < len; ++i) {
    if (check && !checker.checkLabel(keys[i])) {
      throw new SyntaxError('unexpected character of label: "' + keys[i] + '"');
    }

    if (check && !checker.checkValue(recordObj[keys[i]])) {
